...
namespace font
{
    ...
    void desenhaTextoBmp(int x, int y, void *font, string texto)
    {
        glRasterPos2f(x, y);
    ...
    void desenhaTextoStroke(GLfloat x, GLfloat y, void *font, string texto)
    {
        glPushMatrix();
        glTranslatef(x, y, 0);
        glScalef(0.005, 0.005, 0.005);
        for (int i = 0; i < texto.length(); i++)
            glutStrokeCharacter(font, texto.at(i));
        glPopMatrix();
    }
    ...
    void init()
    {
        ...
        window = glutCreateWindow("Exemplo de fonts GLUT bitmap e stroke");
